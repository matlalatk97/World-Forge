    ...
    public class VG_Assemble : MonoBehaviour
    {
        ...
        void Start()
        {
            if (m_assembleArticulation == null)
                VG_Debug.LogWarning("Assemble Articulation is not assigned, so assemble will use Fixed joint for " + transform.name, transform);
            else if (m_assembleArticulation.m_type == VG_JointType.FLOATING)
            {
                VG_Debug.LogError(transform.name + "'s Assemble Articulation can not be FLOATING joint type.", transform);
                this.enabled = false;
            }
                ...
                && originalJointType != VG_JointType.FLOATING)
            {
                ...
                {
                    VG_Debug.LogError(transform.name + "'s initial joint is constrained type " + originalJointType + ", Disassemble Articulation with FLOATING joint type needs to be assigned.", transform);
                    this.enabled = false;
                }
                else if (m_disassembleArticulation.m_type != VG_JointType.FLOATING)
                {
                    VG_Debug.LogError(transform.name + "'s Disassemble Articulation has to be FLOATING joint type.", transform);
                    this.enabled = false;
                }
